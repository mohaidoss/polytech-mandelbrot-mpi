...
int main(int argc, char * argv[]) {
  ...
  if (my_rank == 0) 
  {
    unsigned char * tableau_complet = (unsigned char * ) malloc(w * h * sizeof(unsigned char));
    ...
    for (i = 0; i < h / p; i++) {
      x = xmin;
      for (j = 0; j < w; j++) {
        tableau_complet[j + i * w] = xy2color(x, y, prof);
        x += xinc;
      }
      y += yinc;
    }
    for (int source = 1; source < p; source++) {
      MPI_Recv( & tableau_complet[source * h * w/ p], w * (h / p) * sizeof(unsigned char), MPI_CHAR, source, tag, MPI_COMM_WORLD, & status);
    }

    /* Sauvegarde de la grille dans le fichier resultat "mandel.ras" */
    sauver_rasterfile("mandel.ras", w, h, tableau_complet);
    ...
    ima = (unsigned char * ) malloc(w * (h / p) * sizeof(unsigned char));

    /* Traitement de la grille point par point */
    y = ymin + (my_rank * yinc * h) / p;

    for (i = 0; i < h / p; i++) {
      //fprintf(stderr, "Ligne %d du processus %d",i,my_rank);
      x = xmin;
      for (j = 0; j < w; j++) {
    ...
    MPI_Send(ima, w * (h / p) * sizeof(unsigned char), MPI_CHAR, 0, tag, MPI_COMM_WORLD);
  }
  MPI_Finalize();
}